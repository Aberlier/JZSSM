...
//修改信息
function updateMsgResp(id,loginId,token,role) {
    layer.alert('确定要修改用户信息？', function () {
        $.ajax({
            type:'get',
            url:'respController/updateRespBefore?id='+id+"&loginId="+loginId+"&token="+token+"&role=" +role,
          /*  data:id,//数据为id数组*/
            traditional:true,
    ...
    if(arrays[0].checked){
        layer.confirm('确定要删除该用户信息？', {
            ...
            if ($('#table tbody tr').length === 1) {
                ...
                $.ajax({
                    url: "respController/deleteResp?loginId="+loginId+"&token="+token+"&role=" +role,
                    type: "post",
