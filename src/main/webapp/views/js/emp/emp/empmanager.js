...
$('#fabu').on('click', function () {
    layui.use('layer', function () {
        ...
        layer.open({
            ...
            shade: 0.8,
            area: ['660px', '420px'],
            content: 'views/pages/admin/addEmp.jsp',
            end: function () {
                location.reload();
            }
        });
...
function deleteOne(id,loginId,token,role) {
    layer.confirm('确定要删除该用户信息？', {
        ...
        $.ajax({
            type:'post',
            url:"empController/deleteEmp?loginId="+loginId+"&token="+token+"&id="+id+"&role="+role,
            data:id,//数据为id数组
            traditional:true,
    ...
    if(arrays[0].checked){
        layer.confirm('确定要删除该用户信息？', {
            ...
            if ($('#table tbody tr').length === 1) {
                ...
                $.ajax({
                    url: "empController/deleteEmp?loginId="+loginId+"&token="+token+"&role="+role,
                    type: "post",
